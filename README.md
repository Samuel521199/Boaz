...
**Whitepaper** · 作者：Samuel · 文档更新：2026-02-24
...
| 模块 | 语言/技术 | 说明 |
|------|------------|------|
| **boaz-daemon** 核心守护进程 | Rust | `windows-service` 注册为 NT 服务，开机自启防杀；`sysinfo` 监控进程树；`netstat2` 底层网络连接；Phase 1 已实现「进程与网络感知融合」(The Eye of Boaz) |
| **boaz-shared** IPC 数据结构 | Rust | Daemon 与 UI 通信的共享类型（SuspectMapping、AlertPayload 等） |
| **boaz-agent-ui** 用户交互端 | Tauri | 沿用 boaz-ui，后续扩展为托盘 + 告警弹窗；通过命名管道或本地 WebSocket 与 Daemon IPC 通信 |
...
| 阶段 | 目标 | 状态 |
|------|------|------|
| **Phase 0** | 离线静态审计（当前已实现） | ✅ 已完成 |
| **Phase 1** | 流量与进程感知 | 当前首要任务 |
| **Phase 2** | 挂起与 AI 审讯 | 规划中 |
| **Phase 3** | 联动与自愈 | 规划中 |
...
### Phase 1：流量与进程感知（当前首要任务）✅ 已实现初版

- 实现基于 Rust 的后台进程监控，能够实时抓取「谁在向外发数据」。
- **The Eye of Boaz**：`boaz-daemon` 融合进程图谱与网络连接表，将「哪个文件正在连接哪个外部 IP 的什么端口」映射出来；本地初筛过滤微软签名与白名单，锁定 AppData 下无签名程序外连非常规端口的嫌疑进程。
- **运行测试**：`cargo run -p boaz-daemon`（常驻监控，每 10 秒扫描一次）；`--once` 仅扫一次；`--interval 5` 指定间隔秒数；`--drive D` 或 `--drive C,D` 仅监控指定盘符，不指定则全盘
